...
export class GetSaudeByIdService {
  ...
  public async getSaude(
    ...
    const { registro, specie } = await this.verificarDados(
      usuario,
      idPlanta,
      idRegistro,
    );
  ...
  private verificarDados = async (
    ...
    if (idPlanta && idRegistro)
      throw new GraphQLError(
        "Escolha apenas um tipo de consulta passando apenas um argumento opcional",
      );

    if (!idPlanta && !idRegistro)
      throw new GraphQLError(
        "É necessário passar um registro ou uma planta para a service de relatório de saúde",
      );
      ...
      if (!especie)
        throw new GraphQLError(
          "A espécie da planta nao foi cadastrada no banco de dados",
        );
  ...
  private gerarRelatorioDeSaude(
    registro: IRegistroPlanta,
    specie: Specie,
  ): IRelatorioSaude {
    ...
    const nitrogenio = avaliarSaude(
      Number(registro.nitrogenio),
      v.nitrogenio.min,
      v.nitrogenio.max,
      "nitrogênio",
    );
    const fosforo = avaliarSaude(
      Number(registro.fosforo),
      v.fosforo.min,
      v.fosforo.max,
      "fósforo",
    );
    const potassio = avaliarSaude(
      Number(registro.potassio),
      v.potassio.min,
      v.fosforo.max,
      "potássio",
    );
    const temperatura = avaliarSaude(
      Number(registro.temperatura),
      v.temperatura.min,
      v.temperatura.max,
      "temperatura",
    );
    const umidade = avaliarSaude(
      Number(registro.umidade),
      v.umidade.min,
      v.umidade.max,
      "umidade",
    );
